    ...
    const submithandler=(e)=>{
        ...
        .then((res) => {
            if (res.status === 200) {
               ...
               //         
               const role = localStorage.getItem('role')?.trim(); // Ensure no extra spaces
               console.log(`Role from localStorage: "${role}"`); // Debug log
               
               if (role === "Lecturer") {
                nav('/lecturemenu?tab=studentdetails'); // Navigate to lecture if role is Lecturer
               } else {
                   nav('/headmenu?tab=students'); // Navigate to headmenu otherwise
               }
        ...
        return (
            <>
            <br/><div id="stu1" style={{ display: "flex", gap: "0px" ,marginLeft:"60px",marginTop:"-10px"}}>
            <div> 
